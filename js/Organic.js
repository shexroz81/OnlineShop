...
async function fetchProducts(searchTerm = "") {
  ...
  try {
    ...
    allProducts = data.products || [];  // products massivi bo'lsa oladi, bo'lmasa bo'sh massiv
    renderProducts(sortProducts(allProducts));
    updateHeader(searchTerm || "All Products", allProducts.length);
  } catch (err) {
    console.error("Fetch error:", err); // Internet yo'q yoki server xato bo'lsa
    allProducts = [];
    renderProducts([]);
...
function createCard(product, index) {
  ...
  card.innerHTML = `
    ...
    <div class="product-info">
      ...
      <div class="product-rating">${stars} <span>(${
    product.rating || "4.0"
  })</span></div>
...
function removeFromCart(id) {
  // Berilgan ID ga teng bo'lmagan mahsulotlarni qoldiramiz (filtrlash)
...
function updateCartUI() {
  ...
  cartTotal.textContent = `$${total.toFixed(2)}`; // Savat pastidagi jami
...
function doSearch() {
  ...
  fetchProducts(term); // Qidiruv so'zi bo'yicha mahsulot yuklaydi
}
